...
export class AddDebitorComponent implements OnInit{
  ...
  constructor(private fb: FormBuilder,private  dialogRef : MatDialogRef<AddDebitorComponent>, @Optional() @Inject(MAT_DIALOG_DATA) private dialogData : any ){
    this.formDispositors = this.fb.group({
      ...
      postleitzahl: [Number, [Validators.required, Validators.pattern(GlobalValidators.postleitReq)]],
      uStIdentifikationsnummer: ['', Validators.required]
  });
  }
  ngOnInit(): void {
   if(this.dialogData !== undefined && this.dialogData !== null && this.dialogData.id !== null){
