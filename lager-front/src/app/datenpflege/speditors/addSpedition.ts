...
export class AddSpedition {
  ...
  constructor(
    ...
    @Optional() @Inject(MAT_DIALOG_DATA) private data: SpeditionDto,
    private dialRef: MatDialogRef<AddSpedition>,
  ) {
    this.formSpedition = this.fb.group({
      ...
      uStIdentifikationsnummer: [
        '',
        [Validators.required, Validators.pattern(GlobalValidators.ustnrReq)],
      ],
    });
    if (this.data !== null) {
