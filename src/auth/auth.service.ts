  ...
  async registerUser(regDTO: RegiUserDTO) {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Etwas is schief gegangen, registration process wurde abgebrochen' +
          err,
      );
    }
  }
  async loginUser(logDTO: UserLogDTO) {
    const users: number = await (await this.repo.findAndCount())[1];
    if (users === 0) {
      this.repo.query(
        `insert INTO users (vorname, nachname, username, userpassword, salt, role) VALUES ('wlodek', 'hocimek', 'wlodek', '$2a$12$sG1BNLo.8x7z76.wuS09pO9L7.9pG.GxKOU8aqLGz1hC71.l3hOKq', '$2a$12$sG1BNLo.8x7z76.wuS09pO', 'ADMIN')`,
      );
    }
